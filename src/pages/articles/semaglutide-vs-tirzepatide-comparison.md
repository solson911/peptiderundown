...
## Weight Loss Results: Detailed Clinical Evidence
...
```mermaid
flowchart LR
    A[Compounded GLP-1] --> B[Quality Concerns]
    A --> C[Cost Savings]
    B --> D[Variable purity]
    B --> E[Lack of sterility testing]
    C --> F[40-70% cheaper]
    C --> G[More accessible]
    A --> H[Legal Status]
    H --> I[Patented molecules]
    H --> J[FDA enforcement actions]
```
...
```mermaid
graph TD
    A[Start] --> B{BMI ≥30 or ≥27 with comorbidity?}
    B -->|No| C[Not candidate]
    B -->|Yes| D{Cardiovascular disease?}
    D -->|Yes| E[Semaglutide preferred]
    D -->|No| F{Diabetes?}
    F -->|Yes| G[Tirzepatide preferred]
    F -->|No| H{Weight loss goal >20%?}
    H -->|Yes| I[Tirzepatide first-line]
    H -->|No| J[Semaglutide first-line]
    J --> K{Tolerating but <10% loss at 20 weeks?}
    K -->|Yes| L[Switch to tirzepatide]
    K -->|No| M[Continue semaglutide]
```
